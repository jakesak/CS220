...
def star_cell(row_idx, col_name, stars_rows, header=stars_header):
    ...
    if val == "":
        return None
    elif col_name == "Stellar Effective Temperature [K]" or col_name == "Stellar Radius [Solar Radius]" \
    or col_name == "Stellar Mass [Solar mass]" or col_name == "Stellar Luminosity [log(Solar)]" \
    or col_name == "Stellar Surface Gravity [log10(cm/s**2)]" or col_name == "Stellar Age [Gyr]":
        return float(val)
    else:
        return val
...
for file in files_in_data:
    if file.startswith("stars"):
        star_dict = get_stars(file)
        stars_dict.update(star_dict)
...
def planet_cell(row_idx, col_name, planets_rows, header=planets_header):
    ...
    if col_name in ["Controversial Flag"]:
        ...
        if val == "0":
            return False
    elif col_name == "Orbital Period [days]" or col_name == "Planet Radius [Earth Radius]" \
    or col_name == "Planet Mass [Earth Mass]" or col_name == "Orbit Semi-Major Axis [au]" \
    or col_name == "Eccentricity" or col_name == "Equilibrium Temperature [K]" \
    or col_name == "Insolation Flux [Earth Flux]":
        return float(val)
...
def get_planets(planet_file, mapping_file):
    planets_csv = process_csv(os.path.join("data", planet_file))
...
def get_planetsv2(planet_file, mapping_file, planet_path):
    planets_csv = process_csv(os.path.join(planet_path, planet_file))
    planets_header = planets_csv[0]
    planets_rows = planets_csv[1:]
    try:
...
for idx in range(len(planet_files)):
    planet_data = get_planets(planet_files[idx], mapping_files[idx])
    planets_list.extend(planet_data)
...
for planet in planets_list:
    ...
    if orbital_period == None or semi_major_radius == None:
        continue
    if host == "GJ 9827":
        orbitals = orbital_period**2
        radii = semi_major_radius**3
        ratio = orbitals/radii
...
temp_4th_power_list = []
for planet in planets_list:
    flux = planet.insolation_flux
    temp = planet.equilibrium_temperature
    if flux == None or temp == None:
        continue
    flux_list.append(flux)
    temp_4th_power_list.append(temp**4)
...
for planet in planets_list:
    flux = planet.insolation_flux
    temp = planet.equilibrium_temperature
    if flux == None or temp == None or flux > 7000:
        continue
    flux_list_no_outliers.append(flux)
    temp_4th_power_list_no_outliers.append(temp**4)
...
for star in stars_dict:
    ...
    if stellar_mass == None or stellar_mass < 0.3:
        continue
    if stellar_mass < 8:
        star_class = "Red Giant"
    elif stellar_mass < 10.5:
        star_class = "White Dwarf"
    else:
        star_class = "Neutron Star"
    if star_class not in star_classes:
        star_classes[star_class] = []
    star_classes[star_class].append(star)
...
for star_class in star_classes:
    ...
    for star in star_classes[star_class]:
        ...
        if luminosity != None:
            ...
            num += 1
    avg_lum = lum/num
    star_classes_avg_lum[star_class] = avg_lum    
star_classes_avg_lum
...
for star_class in star_classes:
    ...
    for star in star_classes[star_class]:
        ...
        luminosity = stars_dict[star].stellar_luminosity
        if mass == None or radius == None or luminosity == None:
            continue
        lum_dict[star_class].append(luminosity)
...
for star_class in star_classes:
    ...
    for star in star_classes[star_class]:
        ...
        if density > 25:
            continue
        lum_dict_no_outliers[star_class].append(luminosity)
...
def get_all_paths_in(directory):
    ...
    for content in os.listdir(directory):
        ...
        if os.path.isdir(os.path.join(directory, content)):
            sub_paths = get_all_paths_in(os.path.join(directory, content))
            paths_list.extend(sub_paths)
        elif os.path.isfile(os.path.join(directory, content)):
...
"broken_data", "keplers", "kepler_100s"
grader.check("q12")
...
for path in broken_data:
    planets = get_planetsv2("planets_5.csv", path, "data")
...
gj_674_b_planet = None
for planet in all_planets_list:
    if planet.planet_name == "GJ 674 b":
        gj_674_b_planet = planet
        break
gj_674_b_gravity = get_surface_gravity(gj_674_b_planet)
gj_674_b_gravity
...
def get_distances_to_star(planet):
    ...
    semi_major_radius = planet.semi_major_radius
    if eccentricity == None or semi_major_radius == None:
        return None
    else:
        shortest_distance = semi_major_radius*(1-abs(eccentricity))
        longest_distance = semi_major_radius*(1+abs(eccentricity))
        distances = [shortest_distance, longest_distance]
        return distances
...
b_cen_ab_b_planet = None
for planet in all_planets_list:
    if planet.planet_name == "b Cen AB b":
        b_cen_ab_b_planet = planet
        break
distances_to_star_b_cen_ab_b = get_distances_to_star(b_cen_ab_b_planet)
...
def get_liquid_water_distances(planet):
    host = planet.host_name
    stellar_lum = stars_dict[host].stellar_luminosity
    if stellar_lum == None:
        return None
    else:
        shortest = ((10**stellar_lum)/1.15)**(1/2)
        longest = ((10**stellar_lum)/0.53)**(1/2)
        distances = [shortest, longest]
        return distances
...
kepler_197_e_planet = None
for planet in all_planets_list:
    if planet.planet_name == "Kepler-197 e":
        kepler_197_e_planet = planet
        break
liquid_water_distances_kepler_197_e = get_liquid_water_distances(kepler_197_e_planet)
...
for planet in all_planets_list:
    ...
    if eccentricity == None or semi_major == None or stellar_lum == None:
        continue
    star_distances = get_distances_to_star(planet)
    shortest_dist = star_distances[0]
    longest_dist = star_distances[1]
    liquid_distances = get_liquid_water_distances(planet)
    shortest_liquid_dist = liquid_distances[0]
    longest_liquid_dist = liquid_distances[1]
    if shortest_dist >= shortest_liquid_dist and longest_dist <= longest_liquid_dist:
        planets_with_liquid_water.append(name)
planets_with_liquid_water
...
def get_surface_temperatures(planet):
    ...
    if temp == None:
        ...
        min_temp = ((1-0.5)**(1/4))*temp
        max_temp = ((1-0)**(1/4))*temp
...
hd_20794_d_planet = None
for planet in all_planets_list:
    if planet.planet_name == "HD 20794 d":
        hd_20794_d_planet = planet
        break
surface_temp_hd_20794_d = get_surface_temperatures(hd_20794_d_planet)
...
for planet in all_planets_list:
    ...
    if surface_gravity > 0.75 and surface_gravity < 1.25:
        grav_force_planets.append(name)
...
for planet in all_planets_list:
    ...
    if min_temp > 200 and max_temp < 350:
        temp_planets.append(name)

# compute and store the answer in the variable 'habitable_planets', then display it
habitable_planets = []
for planet in all_planets_list:
    name = planet.planet_name
    if name in grav_force_planets and name in temp_planets and name in planets_with_liquid_water:
