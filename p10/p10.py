...
for star in stars_1_dict:
    ...
    if stellar_luminosity != None:
        stellar_lum += stellar_luminosity
        total_stars += 1
avg_lum_stars_1 = stellar_lum/total_stars
avg_lum_stars_1
grader.check("q7")
...
for star in stars_2_dict:
    ...
    if stellar_age != None:
        total_stellar_age += stellar_age
        total_stars += 1
avg_age_stars_2 = total_stellar_age/total_stars
avg_age_stars_2
grader.check("q8")
...
for star in stars_dict:
    if stars_dict[star].stellar_radius != None:
        if biggest_rad == None or stars_dict[star].stellar_radius > biggest_rad:
            biggest_rad = stars_dict[star].stellar_radius
            biggest_star = star
...
planets_1 = get_planets("planets_1.csv", "mapping_1.json")
...
planets_2 = get_planets("planets_2.csv", "mapping_2.json")
...
planets_4 = get_planets("planets_4.csv", "mapping_4.json")
last_five_planets_4 = planets_4[-5:]
last_five_planets_4
grader.check("q15")

planets_5 = get_planets("planets_5.csv", "mapping_5.json")
planets_5
    ...
    if star.stellar_radius != None:
        if star.stellar_radius > 10:
            if planet.planet_radius != None:
                ...
                num += 1
avg_planet_radius_big_stars = total_radius/num
...
for star in stars_dict:
    ...
    if age != None:
        if youngest_age == None or age < youngest_age:
            youngest_age = age
            youngest_star = star
youngest_star_planets = []
for planet in planets_list:
    host = planet.host_name
    if host == youngest_star:
