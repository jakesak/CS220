...
def regression_line_plot(df, x, y):
    ...
    df.plot.line(ax = ax, x = x, y = "fit", color = "r")
...
url = 'https://git.doit.wisc.edu/cdis/cs/courses/cs220/cs220-f22-projects/-/raw/main/p13/QSranking.json'
download(url, 'QSranking.json')
...
num_of_all_institutions = pd.read_sql("""
...
ORDER BY `num_of_institutions` DESC, `country` ASC
""", conn)
other_inst = num_of_all_institutions.iloc[10:].sum()[1]
other_inst_dict = {"country": "Other", "num_of_institutions": other_inst}
num_institutions = num_of_all_institutions.iloc[:10].append(other_inst_dict, ignore_index=True)
...
cit_per_inter_score = pd.read_sql("""
...
overall_score = 72
regression_coeff = get_regression_coeff(inverse_overall_rank, "inverse_overall_score", "rank")
rank_score_72 = math.ceil(regression_coeff[0] * (1/overall_score) + regression_coeff[1])
rank_score_72
