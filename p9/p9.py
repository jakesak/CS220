...
def get_raw_movies(path):
    ...
    for idx in range(len(rows)):
        ...
        for name in column_names:
            output = rows[idx][column_names.index(name)]   
            if name == "year" or name == "duration":
                movie[name] = int(output)
            elif name == "rating":
                movie[name] = float(output)
            elif name == "genres" or name == "directors" or name == "cast":
                movie[name] = output.split(", ")
            else:
                movie[name] = output
        raw_movie_list.append(movie)
...
def get_movies(movies_path, mapping_path):
    ...
    for movie in movies_data:
        for key in movie:
            if key == "title": 
                title = mapping_dict[movie[key]]
                movie[key] = title
            elif key == "cast" or key == "directors":
                new_list = []
                for ids in movie[key]:
                    name = mapping_dict[ids]
...
harry_potter_movies = find_specific_movies(movies, "Harry Potter")
yate_potter_movies = find_specific_movies(harry_potter_movies, "David Yates")
hp_ratings = []
for movie in yate_potter_movies:
    rating = movie["rating"]
    hp_ratings.append(rating)
median_hp_rating = median(hp_ratings)
median_hp_rating
...
#drama and comedy are by far the most popular, surprising the gap between these 2 and the rest
...
kurosawa_genre_buckets = bucketize(kurosawa_movies, "genres")
kurosawa_genres = {genre: len(kurosawa_genre_buckets[genre]) for genre in kurosawa_genre_buckets}
kurosawa_genres
...
for year in year_buckets:
    ...
    for idx in range(len(year_buckets[year])):
        decade_buckets[decade].append(year_buckets[year][idx])
decade_mapping = {decade: len(decade_buckets[decade]) for decade in decade_buckets}
decade_mapping
...
median_genres = {genre: median(genre_ratings[genre]) for genre in genre_ratings}
median_genres
...
genres_desc = sorted(median_genres, key = median_genres.get, reverse = True)
genres_desc
...
shrek_animation = find_specific_movies(shrek_movies, "Animation")
shrek_inc = sorted(shrek_animation, key = lambda movie: movie["year"])
shrek_inc
...
denzel_buckets = bucketize(cast_buckets["Denzel Washington"], "directors")
denzel_buckets
denzel_director_len = {director: len(denzel_buckets[director]) for director in denzel_buckets}
denzel_directors = sorted(denzel_director_len, key = denzel_director_len.get, reverse = True)
denzel_directors
